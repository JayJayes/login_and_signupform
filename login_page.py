...
class login_page:
    def __init__(self, root):
        ...
        frame.place(x=350, y=150, width=500, height=500)

        # Title
        title1 = Label(frame, text="Log In", font=("times new roman", 25, "bold"), bg="white").place(x=20, y=10)
        title2 = Label(frame, text="Welcome Back!", font=("times new roman", 13), bg="white", fg="gray").place(x=20, y=50)

        # Email
        email_label = Label(frame, text="Email Address", font=("helvetica", 15, "bold"), bg="white").place(x=20, y=100)
        self.email_entry = Entry(frame, font=("arial"))
        self.email_entry.place(x=20, y=130, width=420)

        # Password
        password_label = Label(frame, text="Password", font=("helvetica", 15, "bold"), bg="white").place(x=20, y=180)
        self.password_entry = Entry(frame, font=("arial"), show="*")
        self.password_entry.place(x=20, y=210, width=420)

        # Log In Button
        self.login_button = Button(frame, text="Log In", command=self.login_func, font=("times new roman", 18, "bold"),
                                   bd=0, cursor="hand2", bg="blue", fg="white").place(x=120, y=260, width=250)

        # Forgot Password Button
        self.forgotten_pass = Button(frame, text="Forgotten password?", command=self.forgot_func,
                                     font=("times new roman", 10, "bold"), bd=0, cursor="hand2",
                                     bg="white", fg="blue").place(x=160, y=310, width=150)

        # Create Account Button
        self.create_button = Button(frame, text="Create New Account", command=self.redirect_window,
                                    font=("times new roman", 18, "bold"), bd=0, cursor="hand2",
                                    bg="green2", fg="white").place(x=120, y=360, width=250)
        ...
        if self.email_entry.get() == "":
            ...
            try:
                ...
                if row is None:
                    ...
                    Label(self.root, text="Change your password", font=("times new roman", 20, "bold"),
                          bg="white").place(x=10, y=10)
                    Label(self.root, text="It's quick and easy", font=("times new roman", 12),
                          bg="white").place(x=10, y=45)

                    Label(self.root, text="Select your question", font=("times new roman", 15, "bold"),
                          bg="white").place(x=10, y=85)

                    self.sec_ques = ttk.Combobox(self.root, font=("times new roman", 13), state='readonly', justify=CENTER)
                    self.sec_ques['values'] = ("Select", "What's your pet name?", "Your first teacher name",
                                               "Your birthplace", "Your favorite movie")
                    self.sec_ques.place(x=10, y=120, width=270)
                    self.sec_ques.current(0)

                    Label(self.root, text="Answer", font=("times new roman", 15, "bold"),
                          bg="white").place(x=10, y=160)
                    self.ans = Entry(self.root, font=("arial"))
                    self.ans.place(x=10, y=195, width=270)

                    Label(self.root, text="New Password", font=("times new roman", 15, "bold"),
